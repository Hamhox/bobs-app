...
const lowBacklightPalette = createVoyagerScreenPalette({ displayMode: "NORMAL", backlightActive: true, backlightLevel: "LOW" });
const mediumBacklightPalette = createVoyagerScreenPalette({ displayMode: "NORMAL", backlightActive: true, backlightLevel: "MEDIUM" });
const highBacklightPalette = createVoyagerScreenPalette({ displayMode: "NORMAL", backlightActive: true, backlightLevel: "HIGH" });
const timedOutHighBacklightPalette = createVoyagerScreenPalette({ displayMode: "NORMAL", backlightActive: false, backlightLevel: "HIGH" });
const invertedHighBacklightPalette = createVoyagerScreenPalette({ displayMode: "INVERTED", backlightActive: true, backlightLevel: "HIGH" });
...
assert(lowBacklightPalette.screen === normalPalette.screen && lowBacklightPalette.ink === normalPalette.ink, "Low backlight no longer uses the approved reflective LCD palette");
assert(mediumBacklightPalette.screen === "rgb(250 250 247)" && mediumBacklightPalette.ink === "rgb(18 16 17)", "Medium backlight does not apply the midpoint contrast palette");
assert(highBacklightPalette.screen === "rgb(255 255 255)" && highBacklightPalette.ink === "rgb(0 0 0)", "High backlight is not true black and white");
assert(timedOutHighBacklightPalette === normalPalette, "a timed-out High backlight does not return to unlit LCD colors");
assert(invertedHighBacklightPalette.screen === "rgb(0 0 0)" && invertedHighBacklightPalette.ink === "rgb(255 255 255)", "High inverted backlight is not true white on black");
...
assert(createVoyagerPowerProfile({ backlightLevel: "LOW" }).backlightOpacity === 1, "Low backlight does not composite its authored color layer fully");
assert(createVoyagerPowerProfile({ backlightLevel: "MEDIUM" }).backlightOpacity === 1, "Medium backlight does not composite its authored color layer fully");
